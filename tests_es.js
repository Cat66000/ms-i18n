...
describe('ms.format(long string)', () => {
    it('should not throw an error', () => {
        expect(() => {
            ms.format('53 milisegundos');
        }).to.not.throwError();
    });

    it('should convert milisegundos to ms', () => {
        expect(ms.format('53 milisegundos')).to.be(53);
    });
    ...
    it('should convert días to ms', () => {
        expect(ms.format('2 días')).to.be(172800000);
    });
    ...
    it('should work with negative integers', () => {
        expect(ms.format('-100 milisegundos')).to.be(-100);
    });
...
describe('ms.format(number, { long: true })', () => {
    ...
    it('should support milisegundos', () => {
        expect(ms.format(500, { long: true })).to.be('500 milisegundos');
    ...
    it('should support días', () => {
        expect(ms.format(24 * 60 * 60 * 1000, { long: true })).to.be('1 día');
        expect(ms.format(24 * 60 * 60 * 1200, { long: true })).to.be('1 día');
        expect(ms.format(24 * 60 * 60 * 10000, { long: true })).to.be('10 días');

        expect(ms.format(-1 * 24 * 60 * 60 * 1000, { long: true })).to.be('-1 día');
        expect(ms.format(-1 * 24 * 60 * 60 * 1200, { long: true })).to.be('-1 día');
        expect(ms.format(-1 * 24 * 60 * 60 * 10000, { long: true })).to.be('-10 días');
    });

    it('should round', () => {
        expect(ms.format(234234234, { long: true })).to.be('3 días');
